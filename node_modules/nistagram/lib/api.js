...
class Api {
  async getProfile(username) {
    let data = await this.fetchApi(_config.default.GRAPHQL_URL.checkProfile(username), 'GET');
    return data;
  }
  ...
  async following({
    ...
    let {
      data
    } = await this.fetchApi(_config.default.GRAPHQL_URL.fetchFollowing(this.userId, nextCursor));
    let {
      count,
      page_info: {
        has_next_page,
  ...
  async getTimelineStory() {
    let {
      data
    } = await this.fetchApi(_config.default.GRAPHQL_URL.storyTimeline, 'GET');
  ...
  async getTimeLineFeed() {
    let {
      data
    } = await this.fetchApi(_config.default.GRAPHQL_URL.timelineFeed, 'GET');
    let {
      edges
  ...
  async getMediaByUsername(username, countImage = 2) {
    ...
    } = await this.fetchApi(_config.default.GRAPHQL_URL.checkProfile(username), 'GET');
    let {
      edges: nodes,
      page_info
  ...
  async _pageMediaByUserName(nextCursor, idUser, countImage) {
    if (nextCursor) {
      let response = await this.fetchApi(_config.default.GRAPHQL_URL.pagingMedia(idUser.id, nextCursor));
      let {
        page_info,
        edges
  ...
  async getStory(username) {
    ...
    let {
      data
    } = await this.fetchApi(_config.default.GRAPHQL_URL.storyByUserId(response.graphql.user.id), 'GET');
  ...
  configurePhoto(path, uploadID) {
    let imageBuffer = _fs.default.createReadStream(path);
  ...
  async like(id) {
    let data = await this.fetchApi(_config.default.GRAPHQL_URL.likeMedia(id), 'POST');
    return data;
  }

  async comment(id, code, comment) {
    let data = await this.fetchApi(_config.default.GRAPHQL_URL.commentMedia(id), 'POST', {
  ...
  async getUserNotFollowBack(username) {
    ...
    let follower = await this.follower({
      username
    });
