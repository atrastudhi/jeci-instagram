...
const j = _requestPromise.default.jar();

class Nistagram extends _multi.default.inherit(_api.default, _utils.default) {
  constructor(session) {
    super(session);
  ...
  async fetchApi(url, method, body, upload) {
    ...
    ;
    const options = {
      uri: _config2.default.BASE_URL + url,
      method,
      jar: j,
      timeout: 15000,
      headers: { ...headers
    ...
    try {
      let response = await (0, _requestPromise.default)(options);
  ...
  async login(username, password) {
    ...
    const enc = _crypto.default.createHash('md5').update(this.username + this.password).digest('hex');
  ...
  setSession(session) {
    ...
    session.session.map(v => {
      let cookie = _requestPromise.default.cookie(v);
  ...
  async sendRequest(endpoint, method, body) {
    let options = {
      uri: _config2.default.API_URL + endpoint,
      headers: {
        'Connection': 'close',
        'Accept': '*/*',
        'Content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        if (this.responseJson.logged_in_user) {
          ...
          _fs.default.writeFileSync(pathLocation, JSON.stringify(newCredential, null, 2));
